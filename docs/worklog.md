...
### 2026-07-13 · Step 14 — REFRAME: drop the probe entirely; H2 = "does SSL transfer across species?" (commit d645a7f).
- **Why the probe is abandoned:** two dead ends converged. (1) The frozen-encoder BCELoss probe collapsed to all-background (Step 13). (2) CLAUDE.md's intended baseline — a **supervised** model trained on the source — is **impossible here**: the Drosophila+zebrafish source is **unlabeled** raw calcium video (no masks, confirmed Step 9). You cannot train a supervised segmentation model without labels. So "SSL-source vs supervised-source" is off the table.
- **The reframed question (what H2 now is):** *How much of the benefit of self-supervised pretraining survives when the pretraining data is a completely different organism?* Compare three encoders, **all fine-tuned identically on mouse** (H1's working `finetune`: correct masks, min-max-normalized input, fixed 80/20 test on 00.00, per fraction):
  1. **from-scratch** (no pretraining) — null floor
  2. **cross-species SSL** (JEPA on Drosophila+zebrafish) — the H2 arm
  3. **same-species SSL** (JEPA on mouse) — H1's v9 encoder, the in-domain ceiling (free — already trained)
- **Metric — transfer ratio** per fraction: `gain = arm − from-scratch`; `ratio = cross_gain / same_gain`. ≈1 → SSL is organism-general (strong result); ≈0 → no cross-species transfer; between → partial, quantified. Only meaningful where in-species SSL actually helps (same_gain > 0); the report suppresses it otherwise.
- **Implementation:** `run_h2` rewritten to **reuse `finetune()`** (the pipeline that works at ~0.9 Dice) — cross-species-pretrained init (`mode=finetune`) vs from-scratch (`mode=supervised_baseline`), across `labeled_fractions`, on the mouse target. Removed the broken `probe_on_target` + the fabricated `_NF_ORGANISM_MAP`. `plot_h1_dice` parameterized (prefix/title/label) so H2 gets its own titled figure. New `scripts/h2_transfer_report.py` overlays all three arms + the transfer-ratio panel (reads H1 log for the same-species arm, H2 log for cross-species). Leakage-free by construction (source ≠ target species). Verified end-to-end on synthetic data; suite 17/17.
- **Requirement for a fair run:** the cross-species encoder must be pretrained at **dstc=32** (fresh, with augmentation) to match H1 v9 — NOT the old dstc=8 H2.v3 checkpoint. Notebook updated to pretrain on `--source-data` (Drosophila+zebrafish) → fine-tune on `--target-data` (mouse 00.00).

### Status
**H2 reframed to the cross-species SSL transfer question and fully rebuilt on H1's working fine-tune pipeline (`d645a7f`); the broken probe is gone.** Deliverable = 3-way Dice-vs-fraction overlay (from-scratch / cross-species-SSL / same-species-SSL) + transfer ratio (`scripts/h2_transfer_report.py`). Same-species arm is free from H1 v9; **need the Kaggle run to pretrain the cross-species encoder at dstc=32 and fine-tune on mouse**, then run the report. Caveat: the transfer ratio is only crisp where in-species SSL beats scratch (v9: only at 100%) — a cleaner denominator needs the pretraining-convergence fix.
