...
### Status
**H1 v8 results are RETRACTED — leakage.** The JEPA pretraining saw the test recording (00.00 was in `--data`), and finetune/eval ran within that single recording against a static mask. Absolute Dice, the label-efficiency curve, and the "halve the labels" claim are all invalid. Nothing about H1 is reportable until: (a) pretraining excludes the test recording(s), and (b) evaluation is cross-recording (train on some recordings, test on a held-out different one). Fix not yet implemented; the pipeline machinery runs, but the experimental design is wrong.
