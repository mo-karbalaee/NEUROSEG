...
### Status
**v8 fully RETRACTED — three independent defects found and fixed (leakage `992de33`, inference-normalization `f91b0df`, GT-mask transpose `a1335f9`).** The transpose is the most consequential: every prior Neurofinder mask/metric was built on transposed labels, so all H1/H2 numbers and the v8 checkpoints are void — a retrain is required. Suite 17/17; standalone GT-viz + inference-viz (mp4, contours) verified aligned to real cells. **Next: Kaggle v9 with all three fixes → the first H1 result whose masks actually correspond to neurons.**
