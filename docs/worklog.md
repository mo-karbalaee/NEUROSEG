...
### Status
Pipeline and baseline are solid (Dice ~0.78 @ 100% labels, clean monotonic fraction curve, all 4×2 runs complete). **Two separable findings:** (1) **positive & kept** — the semi-supervised setup works as intended (pretrain on 100% unlabeled Neurofinder, finetune on a label fraction); the label-efficiency curve shows ~95% of full-label Dice at 50% labels, ~98% at 75% — labels, the real bottleneck, can be roughly halved cheaply. (2) **unsupported (accepted)** — SSL pretraining does not beat the from-scratch baseline at any fraction because JEPA pretraining **overfit** (val_jepa_loss diverged 3.13→5.96); fixing it (best-val checkpoint, more unlabeled data, early-stop) is future work, not required for the label-efficiency story.
